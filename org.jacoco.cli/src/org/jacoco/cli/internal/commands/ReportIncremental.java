 ...
 */
public class ReportIncremental extends Command {
	...
	private IReportVisitor createReportVisitor() throws IOException {
		...
		final Map<String, Float> lineDiff = new HashMap<String, Float>();
		visitors.add(new IncrementalVisitor(getOldSourceLocator()));
	...
	private ISourceFileLocator getOldSourceLocator() {
		final MultiSourceFileLocator multi = new MultiSourceFileLocator(
				tabwidth);
		for (final File f : oldSourceFiles) {
			multi.add(new DirectorySourceFileLocator(f, encoding, tabwidth));
		}
		return multi;
	}
